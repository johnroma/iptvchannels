...
10. **Kodi sync**: `runKodiSync` (`src/server/kodi.ts`) calls Kodi JSON-RPC `PVR.GetChannels` and updates only `channels.contentId` (+ `updatedAt`) used by YAML export. Matching lives in the pure `matchKodiChannels` (`src/lib/kodi-url.ts`): it tries `channels.name` before `channels.tvgName` against the Kodi `label`, case- and whitespace-insensitively. Kodi labels are the CMS display name (`CNN`), not the raw M3U value (`US| CNN FHD`) — matching `tvgName` alone matches nothing on this data. `syncKodiContentIds` (`src/server/channels.ts`) is the `createServerFn` wrapper. Endpoint/auth resolution lives in the pure helper `resolveKodiConnection` (`src/lib/kodi-url.ts`): `KODI_URL` wins over `KODI_HOST`/`KODI_PORT` (default `localhost:8080`), `/jsonrpc` is appended if absent, and a non-blank `KODI_USER` adds an HTTP Basic header with `KODI_PASSWORD`. These vars must be set in **both** `local.env` (dev) and `prod.env` (build + `srv/` runtime).
...
12. **M3U formatting rules**: M3U uses `name || tvgName` as the visible label after the comma, keeps `tvg-name` as the original `tvgName`, and prefers the full English country name derived from `countryCode` for `group-title` before falling back to `COALESCE(group_titles.alias, group_titles.name)`.
13. **YAML feed URL**: `/channels/yaml` (server route in `src/server.ts`) runs `runKodiSync()` first, then serves `getActiveChannelsYaml()` as `text/yaml` with a comment header carrying sync/export stats and skip reasons. A failed sync returns **502** rather than serving stale `contentId` values.

## Navigation Overview

- `Home` - system overview and explanation of the navigation
- `Channels` - live TV management, filters, active/favourite state, `Export M3U`, `Export YAML`, and `Sync Kodi`
- `Channels M3U URL` - direct active-channel playlist at `/channels/m3u`
- `Channels YAML` - direct Home Assistant script YAML at `/channels/yaml` (Kodi sync runs first)
- `Add Channel` - create a new live TV channel
