...
const seriesGroupTitlesQueryOptions = {
  queryKey: ["groupTitles", "series"],
  queryFn: () => getSeriesGroupTitles(),
}
...
export const Route = createFileRoute("/series/")({
  validateSearch: (search) => seriesSearchSchema.parse(search),
  loaderDeps: ({ search }) => ({
    page: search.page,
  ...
  loader: async ({ context, deps }) => {
    ...
    await Promise.all([
      ...
      context.queryClient.ensureQueryData(seriesGroupTitlesQueryOptions),
    ])
  },
...
async function handleToggleSeriesActive(id: string, active: boolean) {
  return toggleSeriesActive({ data: { id, active } })
}
...
function RouteComponent() {
  ...
  const { data: groupTitles } = useSuspenseQuery(
    seriesGroupTitlesQueryOptions,
  )
  ...
  const totalCount = seriesData?.totalCount ?? 0
  const totalPages = Math.ceil(totalCount / 100)

  const exportM3uMutation = useMutation({
    mutationFn: () => exportActiveSeriesM3u(),
    onSuccess: (result) => {
      if (result.count === 0) {
        alert("No active series with stream URLs found to export.")
        return
      }
    ...
    <div className="p-4">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="text-sm text-muted-foreground">
          Showing {allSeries.length} of {totalCount} series
